...
import {
  ...
  ThumbsUp, ThumbsDown, Lightbulb, Upload, ShieldAlert, Search,
} from 'lucide-react';
...
import { funnelFor, closedCount, revenueFrom, leadsForStage, todaySplit, MIN_SAMPLE } from '../../lib/funnel';
import {
...
/** Fields a name/company/title search box checks against, in order of relevance. */
const searchableText = (l: Lead): string =>
  [l.name, l.company_name, l.job_title, l.linkedin_url].filter(Boolean).join(' • ').toLowerCase();
...
export const LinkedInApp: React.FC<{ onExit: () => void }> = ({ onExit }) => {
  ...
  // Which funnel tile is open in the drill-down modal, or null when none is.
  ...
  // Same-day counts for the two tiles worth checking hour to hour, fixed to
  // an IST calendar day regardless of what timezone this device is set to.
  const today = useMemo(() => todaySplit(leads), [leads]);
  ...
  // Search narrows only the browsing list on the left — the funnel tiles and
  // the due queue above it stay a read on every lead, since "how many have I
  // requested" should never quietly change because of text left in a search box.
  const filteredLeads = useMemo(() => {
    const q = search.trim().toLowerCase();
    if (!q) return leads;
    return leads.filter((l) => searchableText(l).includes(q));
  }, [leads, search]);
  ...
  return (
    <div className="min-h-screen flex flex-col app-canvas accent-linkedin">
      ...
      {leads.length > 0 && (
        <div className="max-w-6xl w-full mx-auto px-6 pt-5">
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-2">
            {funnel.map((st) => (
              // A tile, not a div: every count here is a click away from the
              // exact leads it's counting, so "who's in the 39" is answered
              // by the app instead of by memory.
              <button
                key={st.key}
                type="button"
                onClick={() => setStageModal(st.key)}
                className="w-full text-left rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800/50 p-3 hover:border-linkedin-300 dark:hover:border-linkedin-600 transition-colors"
              >
                ...
                {/* All-time is the tile's headline number; these are the only
                    two stages worth a same-day readout, fixed to IST so it
                    reads the same regardless of the viewer's own clock. */}
                {st.key === 'requested' && (
                  <p className="text-[10px] text-linkedin-600 dark:text-linkedin-400 font-semibold mt-0.5">
                    {today.requestedToday} today (IST)
                  </p>
                )}
                {st.key === 'connected' && (
                  <p className="text-[10px] text-linkedin-600 dark:text-linkedin-400 font-semibold mt-0.5">
                    {today.connectedToday} today (IST)
                  </p>
                )}
              </button>
            ))}
          ...
          {leads.length > 0 && (
            <div className="relative mb-3">
              ...
              <input
                ...
                className="input-modern !py-2 !pl-9 text-sm w-full"
              />
            </div>
          )}
          <h2 className="text-sm font-semibold text-gray-500 uppercase tracking-wide mb-3">
            Leads ({filteredLeads.length}
            {search.trim() && filteredLeads.length !== leads.length ? ` of ${leads.length}` : ''})
          </h2>
          {loading ? (
      ...
      {stageModal && (
        <StageLeadsModal
          label={funnel.find((s) => s.key === stageModal)?.label ?? stageModal}
          leads={leadsForStage(leads, stageModal)}
          onClose={() => setStageModal(null)}
          onSelect={(id) => { setSelectedId(id); setStageModal(null); }}
        />
      )}
    </div>
  );
...
/**
 * Who a funnel tile's count actually means. Opened by clicking the tile;
 * picking a name here closes the modal and opens that lead, same as picking
 * one from the "Due now" queue or the main list.
 */
const StageLeadsModal: React.FC<{
  label: string;
  leads: Lead[];
  onClose: () => void;
  onSelect: (id: string) => void;
}> = ({ label, leads, onClose, onSelect }) => (
  <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-4" onClick={onClose}>
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white">{label} ({leads.length})</h3>
        <button onClick={onClose} className="p-1 text-gray-400 hover:text-gray-600" aria-label="Close">
          <X className="w-5 h-5" />
        </button>
      </div>
      {leads.length === 0 ? (
        <p className="text-sm text-gray-500 dark:text-gray-400">No leads here yet.</p>
      ) : (
        <div className="space-y-1.5">
          {leads.map((l) => (
            <button
              ...
              className="w-full text-left px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/50"
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-medium text-gray-900 dark:text-white truncate">{l.name}</span>
                <span className="text-[10px] uppercase font-bold px-1.5 py-0.5 rounded-full bg-linkedin-100 text-linkedin-700 dark:bg-linkedin-900/40 dark:text-linkedin-300 flex-shrink-0">
                  {STATUS_LABELS[l.status]}
                </span>
              </div>
              <span className="block text-[11px] text-gray-500 dark:text-gray-400 truncate">
                {[l.job_title, l.company_name].filter(Boolean).join(' · ') || l.linkedin_url}
              </span>
            </button>
          ))}
        </div>
